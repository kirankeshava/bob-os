...
To ensure uninterrupted service, please add your payment method now:

${billingPortalUrl}

After the trial ends, your subscription will be $97/month and you'll continue to receive:
...
async function checkTrials() {
  ...
  for (const customer of trialCustomers) {
    ...
    if (daysUntilExpiry <= 2 && customer.stripeCustomerId) {
      try {
        ...
        await sendPaymentReminderEmail(customer, session.url ?? domain);
      } catch (err) {
